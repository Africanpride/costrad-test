<div>
    <div class="w-64" x-data="{open:false}" x-on:click.away="open=false">
        <button type="button" class="border border-gray-300 p-2 rounded text-black shadow-inner w-64 flex justify-between items-center text-sm focus:outline-none" x-on:click="open=!open">
            <span class="float-left">{{$Patient->full_name ?? 'Select Patient'}}</span>
        ...
        <div class="absolute z-10 w-64 rounded shadow-md bg-white" x-show="open" x-cloak>
            <ul class="list-reset p-2 max-h-64 overflow-y-auto text-sm">
                ...
                <li class="" wire:click="select({{$item->id}})" x-on:click="open=false" id="Patient-{{$item->id}}">
                    <p class="p-2 w-full text-black hover:bg-gray-300 flex justify-between items-center cursor-pointer @if($Patient->id == $item->id) bg-gray-200 @endif">
                        <span>{{$item->name}}</span>
                ...
                @empty
                <li x-on:click="open=false" id="no-Patient">
                    <p class="p-2 block text-red-800 hover:bg-red-200 cursor-pointer" value="0">{{__('No Patients founds')}}</p>
                </li>
                @endforelse
            </ul>
        </div>
