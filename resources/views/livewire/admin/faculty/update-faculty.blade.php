<x-admin.user-modal formAction="updateFaculty">
    <x-slot name="title">
                ...
                <div class="relative flex items-start">
                    ...
                    <label for="hs-checkbox-delete" class="ml-3">
                        ...
                        <span id="hs-checkbox-delete-description"
                            class="block text-sm text-gray-600 dark:text-gray-500 italic ">{{ $resetPassword ? 'Faculty would be notified to reset Password' : 'Tick here to reset Faculty password' }}</span>
                    </label>
                </div>
                ...
                <div class="relative flex items-start">
                    ...
                    <label for="hs-checkbox-archive" class="ml-3">
                        <span class="block text-sm font-semibold text-gray-800 dark:text-white">Activate/De-activate
                            Faculty Account</span>
                        <span id="hs-checkbox-archive-description"
                            class="block text-sm text-gray-600 dark:text-gray-500 italic ">{{ $active ? 'Faculty Account would be activated.' : 'Tick here to activate faculty account' }}</span>
                    </label>
                </div>
                @error('active')
                <span class="text-red-500">{{ $message }}</span>
                ...
                <div class="w-full grid grid-cols-2 gap-2 text-gray-900 dark:text-gray-100">
                    @foreach ($roles as $key => $role)
                        <div class="flex justify-start items-center space-x-2"
                            ...
                            <input
                                class="border-gray-200 rounded text-blue-600 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800"
                                type="checkbox" value="{{ $role->id }}" wire:model.defer="facultyRoles"
                                name="facultyRoles" value="{{ $role->id }}"
                                @if (in_array($role->id, $facultyRoles)) checked @endif>
